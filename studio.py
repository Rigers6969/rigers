"""Content Studio - one dashboard with two tabs: a psychology voiceover
generator and a shot-list media finder.

    streamlit run studio.py --server.port 8505

Both tools' logic lives directly in this file (voiceover script writing +
text-to-speech) plus the shotsource/ package (media finder). No other
files in this repo are required - see shotsource/ for its own sources,
quality filter, and caching.
"""
...
# --------------------------------------------------------------------------
# Script writing - subclasses implement _call_model only. A 10,000-word
# script is far beyond any single LLM call's reliable output length, so
# the script is planned as an outline and then written section by
# section, each one continuing from the tail of the last, until the
...
# separately, then stitched into one file with ffmpeg.
# --------------------------------------------------------------------------
